...
func (lsn *listenerV2) processRequestsPerSubBatch(
	...
	for i < len(reqs) {
		...
		for ; i < len(reqs); i++ {
			...
			simResult := lsn.simulateFulfillment(ctx, maxGasPriceWei, req, rlog)
			if simResult.err != nil {
				rlog.Warnw("Unable to get max link for fulfillment, skipping request", "err", simResult.err)
				continue
			}
			...
			// batch.
			if (totalGasLimit + simResult.gasLimit) > batchMaxGas {
				break
			}
			...
			totalGasLimit += simResult.gasLimit
